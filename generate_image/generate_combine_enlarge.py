...
complete_success_combination=0
all_combination=0
enlarge_acc=0
enlarge_acc2=0
combine_acc=0
...
for n in range(0,img_num):
    ...
    total_area=num*patch_size*patch_size
    ...
    for myid in top10_index:
        ...
        center_x=wi+Arf/2
        center_y=hi+Arf/2
        tmp,tmpPDF=generate_bubble_image(oimage,center_x,center_y,5,area_size,224)
        myfname=enlarge_dir+image_path[n][findex:-5]+'_enlarge_'+str(num)+'_vc_'+str(myid)+'.jpeg'
        # cv2.imwrite( myfname,tmp)
    if image_path[n][findex:-5]=='n02002556_9531_crop_0_0_0':
        print(top10_index)
        sys.exit(0)
    ...
    #     myfname=combine_dir+image_path[n][findex:-5]+'_combine'+str(num)+'_no_'+str(i)+'.jpeg'
